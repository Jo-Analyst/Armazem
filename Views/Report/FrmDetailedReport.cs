...
namespace Interface
{
    public partial class FrmDetailedReport : Form
    {
        ...
        private void CheckNumberOfPages(int numberRows)
        {
            ...
            pageMaximum = PageData.SetPageQuantityRowsDetailedReport(name, dateEntry);
        ...
        private void LoadReport()
        {
            try
            {
                ...
                DataTable dtReport = DetailedReport.GetReport(name, dateEntry, pageSelected, quantRows);
                ...
                foreach (DataRow storage in dtReport.Rows)
                {
                    ...
                    dgvReport.Rows[index].Cells["ColTotalExit"].Value = storage["total_exit"].ToString();
                    dgvReport.Rows[index].Cells["ColBalance"].Value = storage["balance"].ToString();
        ...
        {
            DataTable report = DetailedReport.GetDetailedReportCompleted(name, dateEntry);
            ...
            reportClone.Columns.Add("total_exit", typeof(string));
            reportClone.Columns.Add("balance", typeof(string));
            ...
            foreach (DataRow row in report.Rows)
            {
               ...
               reportClone.Rows[index]["name"] = row["name"].ToString();
                ...
                reportClone.Rows[index]["total_exit"] = row["total_exit"].ToString();
                reportClone.Rows[index]["balance"] = row["balance"].ToString();
               index++;
            }

            new FrmReportPrint(reportClone, true).ShowDialog();
        }
